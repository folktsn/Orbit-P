  ...
  return (
    <Dialog.Root open={!!selectedCandidate} onOpenChange={(open) => !open && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-white dark:bg-surface shadow-2xl z-50 overflow-y-auto">
          {selectedCandidate && (
            <motion.div
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
            ...
            >
              <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between sticky top-0 bg-white/80 dark:bg-surface/80 backdrop-blur-md z-10">
              ...
              <div className="p-6 flex-1 space-y-8">
                ...
                <div className="flex gap-4">
                  <button className="flex-1 flex items-center justify-center gap-2 bg-action text-action-foreground rounded-xl font-semibold py-2 hover:opacity-90 transition-colors shadow-sm">
                ...
                <div>
                  ...
                  <div className="border border-brand-100 dark:border-brand-500/20 bg-brand-50 dark:bg-brand-500/10 rounded-2xl p-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-medium text-brand-900 dark:text-brand-100 flex items-center gap-2"><Sparkles className="w-4 h-4 text-brand-600 dark:text-brand-400" /> คะแนนความเหมาะสม</span>
                      <span className="text-xl font-bold text-brand-700 dark:text-brand-400">{selectedCandidate.score}%</span>
                    </div>
                    <p className="text-sm text-brand-800/80 dark:text-brand-300/80">ผู้สมัครมีความโดดเด่น ทักษะตรงกับความต้องการสูง แนะนำให้เข้าสู่รอบสัมภาษณ์ทันที</p>
                  </div>
                </div>
